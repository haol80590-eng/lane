...
【车道势能 r_lane】简洁设计（文献常见形式）

    r_lane = r_target + r_deviation

    r_target = R_t · 1_{d=0}                    # 在目标车道时的正奖励
    r_deviation = -α · d · (1 + β · p)          # 偏离目标车道的惩罚

    物理意义：
    - 目标车道是势能最低点（d=0 时有正奖励）
    - 偏离越远、进度越大，惩罚越重
    - 简洁明了，符合论文常见设计
        ...
        for vehicle_id in vehicle_ids:
            ...
            try:
                if vehicle_id in getattr(self, 'newly_exited_this_step', {}):
                    ...
                    # ==================================================
                    if d == 0:
                        # 在目标车道：正奖励（解决正向激励缺失问题）
                        r_lane = R_target * (1 + beta * p)  # 后期正奖励更大
                    else:
                        # 偏离目标车道：惩罚
                        r_lane = -alpha * d * (1 + beta * p)
